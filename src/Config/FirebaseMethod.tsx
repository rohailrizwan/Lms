...
export let fbStudent = (body: any, path: string) => {
  return (
    new Promise((resolve, reject) => {
      if (!body.email || !body.password || !body.name) {
        alert("Email password and name is required")
...
export let FbCourse = (body: any, path?: string) => {
  return (
    new Promise((resolve, reject) => {
      const keyreference = push(ref(db, `Course/${path}`)).key;
      let formid = keyreference
      body.id = formid
      const reference = ref(db, `Course/${path}/${formid}`);
      set(reference, body)
        .then(() => {
          resolve("Form created successfully");
        })
...
export let addquiz = (body: any, path?: string) => {
  return new Promise((resolve, rejects) => {
    const refkey = push(ref(db, `Quiz/${path}`)).key
    const reference = ref(db, `quiz/${path}`)
...
export let Fbgetid = (formName: string, id: any) => {
  return new Promise((resolve, reject) => {
    const reference = ref(db, `${formName}/${id}`)
    onValue(reference, (data) => {
      if (data.exists()) {
        resolve(data.val())
      }
